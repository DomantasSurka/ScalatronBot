package scalatron.botwar.botPlugin.Domas
import scala.collection.mutable.Queue
object ControlFunction
{
    def forMaster(bot: Bot) {
        val (directionValue, nearestEnemyMaster, nearestEnemySlave, nearestYummy, nearestBeast) = analyzeViewAsMaster(bot.view)
        
        var dontFireFoodFetcherUntil = bot.inputAsIntOrElse("dontFireFoodFetcherUntil", -1)
        ...
        var direction = BFSWay(bot, bot.view)
        ...
        if (direction.isZero) direction = XY.fromDirection45(bestDirection45)
        bot.set("lastDirection" -> bestDirection45)
        bot.move(direction)
    
        if (dontFireFoodFetcherUntil < bot.time && bot.energy > 200) {
            nearestYummy match {
                case None => // no-one nearby
                case Some(relPos) => // yummy is nearby
                    ...
                    bot.spawn(unitDelta, "role" -> "FoodFetcher", "target" -> remainder)
                    bot.set("dontFireFoodFetcherUntil" -> (bot.time + 40))
            }
        } else if (dontFireAggressiveMissileUntil < bot.time && bot.energy > 200) {
            nearestEnemyMaster match {
                case None => // no-one nearby
                case Some(relPos) => // a master is nearby
                    val unitDelta = relPos.signum
                    val remainder = relPos - unitDelta // we place slave nearer target, so subtract that from overall delta
                    bot.spawn(unitDelta, "role" -> "AggressiveMissile", "target" -> remainder)
                    bot.set("dontFireAggressiveMissileUntil" -> (bot.time + relPos.stepCount + 1))
            }
        } else if (dontFireDefensiveMissileUntil < bot.time && bot.energy > 200) {
            nearestEnemySlave match {
                ...
                if (relPos.stepCount < 8) {
                    ...
                    val unitDelta = relPos.signum
                    val remainder = relPos - unitDelta // we place slave nearer target, so subtract that from overall delta
                    bot.spawn(unitDelta,"role" -> "DefensiveMissile", "target" -> remainder)
                    bot.set("dontFireAggressiveMissileUntil" -> (bot.time + relPos.stepCount + 1))
                }
            }
        }
    ...
    def forSlave(bot: MiniBot) {
        bot.inputOrElse("role", "Lurking") match {
          case "FoodFetcher"        => reactAsFoodFetcher(bot)
          case "AggressiveMissile"  => reactAsAggressiveMissile(bot)
          case "DefensiveMissile"   => reactAsDefensiveMissile(bot)
          case s: String            => bot.log("unknown role: " + s)
        }
    }
    
    def reactAsFoodFetcher(bot: MiniBot) {
        val way = BFSWay(bot, bot.view)
        val target = bot.inputAsXYOrElse("target", XY(0, 0))
        if (target.isNonZero) {
            val unitDelta = target.signum
            bot.move(unitDelta)
            val remainder = target - unitDelta
    ...
    def BFSWay(bot: Bot, view: View): XY = {
        var visitedPlaces = Set[XY]()
        val queue = new Queue[XY]{}
        queue.enqueue(XY(0, 0))
        var way = Map[XY, XY]()
        val viewRange = view.size / 2
        
        while (queue.size != 0) {
            val current = queue.dequeue()
            for (x <- -1 to 1; y <- -1 to 1) {
                if (!(x == 0 && y == 0)) {
                    val newCurrent = current + XY(x, y)
                    if (newCurrent.x > -viewRange && newCurrent.x < viewRange && newCurrent.y > -viewRange && newCurrent.y < viewRange) {
                        val field = view(newCurrent)
                        if (field != 'W' && field != 's' && field != 'b' && field != 'p') {
                            if (!visitedPlaces.contains(newCurrent)) {
                                queue.enqueue(newCurrent)
                                visitedPlaces += newCurrent
                                way += (newCurrent -> current)
                                if ((bot.generation != 0 && field == 'M') || field == 'P' || field == 'B') {
                                    var result = newCurrent
                                    while (way(result) != XY(0, 0)) result = way(result)
                                    return result
                                }
                            }
                        }
    ...
    def analyzeViewAsMaster(view: View) = {
        ...
        var nearestYummy: Option[XY] = None
        ...
        (directionValue, nearestEnemyMaster, nearestEnemySlave, nearestYummy, nearestBeast)
    }
}
